                ...
                <div class="col-md-12">
                    ...
                    <div class="row">
                        <div class="col-md-6 col-sm-6">
                            <form action="${servletRoot}/createUserTarget" method="post">
                                <div class="form-group">
                                    <label>Target weight</label>
                                    <input id="targetWeightInput" type="text" class="form-control"
                                           name="targetWeight"
                                           placeholder="Your target weight (must be numerical)"
                                           value="${userSession.userTargetWeight}"
                                    ></div>
                                <c:forEach items="${targetNutritionModels}" var="targetNutritionModel">
                                    <div class="form-group">
                                        <label>${targetNutritionModel.name}, unit:${targetNutritionModel.unit}</label>
                                        <input type="text" class="form-control"
                                               name="${targetNutritionModel.name}"
                                               placeholder="${targetNutritionModel.name} with unit ${targetNutritionModel.unit}"
                                               value="${userSession.userTargetNutritions[targetNutritionModel.name]}"
                                        ></div>
                                </c:forEach>
                                <input type="submit" class="button_medium add_top" onclick="return createUserTarget()"
                                       value="Update">
                            </form>
                        </div><!-- End col-md-6 -->
...
<script>
    function createUserTarget() {
        var targetWeight = $('#targetWeightInput').val();
        if (!targetWeight) {
            alert("Please enter your target weight, other fields are not mandatory");
            return false;
        }
        if (isNaN(targetWeight)) {
            alert("Your target weight must be numerical");
            return false;
        }
        return true;
    }
